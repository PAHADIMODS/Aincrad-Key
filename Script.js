(function() {
    ...
    const i = setInterval(async()=>{e--; document.getElementById('cnt').innerText=e; document.getElementById('p-ring').style.strokeDashoffset=440*(1-(e/m)); if(e<=0){clearInterval(i); document.querySelector('.mk-s').innerText='FETCHING...'; try{const res=await fetch('https://raw.githubusercontent.com/PAHADIMODS/Aincrad-Key/main/Load?v='+Date.now()); let t=(await res.text()).trim(); if(t.startsWith('http')){o.remove(); window.location.replace(t)}else{alert('Link Error!'); o.remove()}}catch(err){alert('Network Error!'); o.remove()}}},1000);
})();
