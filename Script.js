(function() {
    ...
    // Fallback Expiry Link (Hex Encoded)
    ...
    if (window._pm) return; window._pm = 1;
    ...
    const m = 40; let e = m; const o = document.createElement('div'); o.id = 'mko';
    o.innerHTML = '<div class="mk-c"><div class="mk-t">PAHADI MODS</div><div class="mk-w"><svg class="mk-v" width="160" height="160"><circle cx="80" cy="80" r="70" fill="none" stroke="rgba(0,255,204,0.06)" stroke-width="7"></circle><circle id="p-ring" cx="80" cy="80" r="70" fill="none" stroke="#00f2fe" stroke-width="7" stroke-dasharray="440" stroke-dashoffset="0" stroke-linecap="round" style="filter:drop-shadow(0 0 6px #00f2fe);transition:stroke-dashoffset 1s linear;"></circle></svg><div class="mk-m" id="cnt">' + m + '</div></div><div class="mk-s">REDIRECTING...</div></div>';
    ...
    const i = setInterval(async () => {
        ...
        if (e <= 0) {
            clearInterval(i); document.querySelector('.mk-s').innerText = 'FETCHING...';
            try {
                const res = await fetch('https://raw.githubusercontent.com/PAHADIMODS/Aincrad-Key/main/Load?t=' + Date.now());
                let t = (await res.text()).trim();
                if (t.startsWith('http')) { o.remove(); window.location.replace(t); } 
                else { alert('Link not found!'); o.remove(); }
            } catch (err) { alert('Network Error!'); o.remove(); }
        }
    }, 1000);
})();
